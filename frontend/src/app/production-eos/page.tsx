...
function auditActorName(entry: AuditTrailEntry) {
  ...
  const fromUser = `${entry.user?.firstName || ''} ${entry.user?.lastName || ''}`.trim();
  return fromUser || entry.user?.email || 'Unknown user';
}
...
export default function ProductionEosPage() {
  ...
  const hydrateReportForEditing = useCallback((report: LoadedReport) => {
    ...
    setLineNotes((report.notes || []).reduce((acc, note) => {
      acc[note.lineGroup] = note.notes || '';
      return acc;
    }, { 'Line 1': '', 'Line 2': '', 'Line 3': '', 'Line 5': '' } as Record<string, string>));
  ...
  }, [activePageTab, loadAuditTrail, loadSubmittedReports]);
            ...
            <>
              ...
              {activePageTab === 'DASHBOARD' && (
                <section
                  ...
                  role="tabpanel"
                  className="production-eos-tab-panel rounded-lg border border-gray-200 bg-white p-6 shadow-sm dark:border-gray-800 dark:bg-gray-900"
                >
                  <div className="flex items-start gap-4">
                    <div className="flex h-12 w-12 items-center justify-center rounded-lg bg-blue-50 text-blue-700 dark:bg-blue-950/50 dark:text-blue-200">
                      <BarChart3 className="h-6 w-6" />
                    </div>
                    <div>
                      <h2 className="text-lg font-bold text-gray-900 dark:text-white">Dashboard</h2>
                      <p className="mt-1 max-w-2xl text-sm text-gray-600 dark:text-gray-300">
                        Production EOS dashboard charts will be added here. Submitted reports and new report entry now live in their own tabs so dashboard work can grow cleanly.
                      </p>
                    </div>
                  </div>
                </section>
                  ...
                  {!selectedReportId ? (
                    ...
                    <div className="space-y-3">
                      {auditTrail.map((entry) => {
                        ...
                        const hasVisibleAuditDetails = visibleFieldChangeCount > 0;
                        return (
                          <article key={entry.id} className="rounded-lg border border-gray-200 bg-white p-4 shadow-sm dark:border-gray-800 dark:bg-gray-950">
                            <div className="flex flex-col gap-2 md:flex-row md:items-start md:justify-between">
                              <div>
                                <h3 className="text-sm font-bold text-gray-900 dark:text-gray-100">{changes.eventLabel || entry.action}</h3>
                                <p className="mt-0.5 text-xs font-medium text-gray-500 dark:text-gray-400">
                                  {formatDateTime(entry.createdAt)} by {auditActorName(entry)}
                                </p>
                              </div>
                              <div className="flex flex-wrap gap-1.5 text-[11px] font-bold">
                                <span className="rounded bg-blue-50 px-2 py-1 text-blue-700 dark:bg-blue-950 dark:text-blue-200">{visibleFieldChangeCount} field changes</span>
                                <span className="rounded bg-emerald-50 px-2 py-1 text-emerald-700 dark:bg-emerald-950 dark:text-emerald-200">{lineChanges.length} line rows</span>
                                <span className="rounded bg-amber-50 px-2 py-1 text-amber-700 dark:bg-amber-950 dark:text-amber-200">{noteChanges.length} notes</span>
                              </div>
                            </div>

                            {hasVisibleAuditDetails ? (
                            <div className="mt-3 max-h-[360px] overflow-y-auto rounded-md border border-gray-200 dark:border-gray-800">
                              <table className="w-full min-w-[860px] table-fixed text-left text-xs">
                                ...
                                <thead className="bg-gray-100 text-[10px] uppercase text-gray-500 dark:bg-gray-800 dark:text-gray-300">
                                  <tr>
                                    <th className="px-3 py-2">Area</th>
                                    <th className="px-3 py-2">Field</th>
                                    <th className="px-3 py-2">Previous value</th>
                                    <th className="px-3 py-2">Changed value</th>
                                  </tr>
                                </thead>
                                <tbody className="divide-y divide-gray-100 dark:divide-gray-800">
                                  {reportChanges.map((change: any) => (
                                    <tr key={`report-${change.field}`}>
                                      <td className="px-3 py-2 font-semibold text-gray-700 dark:text-gray-200">Report</td>
                                      <td className="px-3 py-2 text-gray-600 dark:text-gray-300">{change.label}</td>
                                      <td className="break-words px-3 py-2 text-gray-500 dark:text-gray-400">{formatAuditValue(change.previousValue)}</td>
                                      <td className="break-words px-3 py-2 font-semibold text-gray-900 dark:text-gray-100">{formatAuditValue(change.currentValue)}</td>
                                    </tr>
                                  ))}
                                  {lineChanges.flatMap((row: any) => row.fields.map((change: any) => (
                                    <tr key={`line-${row.key}-${change.field}`}>
                                      <td className="px-3 py-2 font-semibold text-gray-700 dark:text-gray-200">{row.location || row.key}</td>
                                      <td className="px-3 py-2 text-gray-600 dark:text-gray-300">{change.label}</td>
                                      <td className="break-words px-3 py-2 text-gray-500 dark:text-gray-400">{formatAuditValue(change.previousValue)}</td>
                                      <td className="break-words px-3 py-2 font-semibold text-gray-900 dark:text-gray-100">{formatAuditValue(change.currentValue)}</td>
                                    </tr>
                                  ...
                                  {noteChanges.flatMap((row: any) => row.fields.map((change: any) => (
                                    <tr key={`note-${row.key}-${change.field}`}>
                                      <td className="px-3 py-2 font-semibold text-gray-700 dark:text-gray-200">{row.location || row.key}</td>
                                      <td className="px-3 py-2 text-gray-600 dark:text-gray-300">{change.label}</td>
                                      <td className="break-words px-3 py-2 text-gray-500 dark:text-gray-400">{formatAuditValue(change.previousValue)}</td>
                                      <td className="break-words px-3 py-2 font-semibold text-gray-900 dark:text-gray-100">{formatAuditValue(change.currentValue)}</td>
                              ...
                              <div className="mt-3 rounded-md border border-emerald-200 bg-emerald-50 px-3 py-2 text-xs font-semibold text-emerald-800 shadow-sm dark:border-emerald-900 dark:bg-emerald-950/40 dark:text-emerald-200">
              ...
              {activePageTab === 'NOTES' && (
                ...
                >
                  <div className="mb-5">
                    ...
                    <p className="text-sm text-gray-600 dark:text-gray-400">Safety, quality, and line notes stay with the next submitted report.</p>
                  </div>
                  <div className="grid gap-4 md:grid-cols-2">
                    <label className="block">
                      <span className="mb-1 block text-sm font-semibold text-gray-700 dark:text-gray-200">Safety Concerns / Incidents</span>
                      <textarea value={safetyConcerns} onChange={(event) => setSafetyConcerns(event.target.value)} rows={6} className={editableClass} placeholder="N/A" />
                    </label>
                    <label className="block">
                      <span className="mb-1 block text-sm font-semibold text-gray-700 dark:text-gray-200">Quality Issues / Holds</span>
                      <textarea value={qualityIssues} onChange={(event) => setQualityIssues(event.target.value)} rows={6} className={editableClass} placeholder="N/A" />
                    </label>
                  </div>
                    ...
                    {NOTE_LINES.map((lineGroup) => (
                      <label key={lineGroup} className="block">
                        ...
                        <textarea
                          ...
                          onChange={(event) => setLineNotes((prev) => ({ ...prev, [lineGroup]: event.target.value }))}
                          rows={5}
