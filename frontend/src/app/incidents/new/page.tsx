...
const AIEnhanceButton = ({ 
  ...
  return (
    <button
      ...
      onClick={onClick}
      disabled={isLoading}
      className="inline-flex items-center gap-1.5 px-3 py-1 text-xs font-medium rounded-full bg-gradient-to-r from-purple-500 to-indigo-500 text-white hover:from-purple-600 hover:to-indigo-600 transition-all disabled:opacity-50 disabled:cursor-not-allowed shadow-sm hover:shadow-md"
    >
      {isLoading ? (
        <>
...
function NewIncidentPageContent() {
  ...
  const loadExistingIncident = async (id: string) => {
    try {
      ...
      // Allow editing DRAFT incidents OR Workplace Safety incidents with pending Investigation
      const isWorkplaceSafety = incident.type === 'WORKPLACE_SAFETY';
      const investigationPending = isWorkplaceSafety && !incident.investigationSubmittedAt;
      const canEdit = incident.status === 'DRAFT' || investigationPending;
      
      if (!canEdit) {
        setError('This incident cannot be edited');
        setLoading(false);
        return;
      }
  ...
  const generateAISummary = async () => {
    if (!formData.description || formData.description.trim().length < 10) {
      setError('Please enter a description (at least 10 characters) before generating AI summary');
      return;
    }
    ...
    try {
      ...
      setError(err.response?.data?.error || 'Failed to generate AI summary. Please check with your system administrator.');
      setFormData(prev => ({ ...prev, aiSummary: '' }));
    } finally {
  ...
  const handleSaveProgress = useCallback(async (showMessage: boolean = true) => {
    ...
    if (!formData.type || !formData.categoryId || !formData.description || !formData.facilityId) {
      if (showMessage) {
        setSaveMessage({ type: 'error', text: 'Please fill in the required fields (Type, Category, Description, Facility) before saving.' });
  ...
  const ValidationResultsPanel = ({ 
    ...
    <div className="mt-6 p-4 bg-gradient-to-r from-blue-50 to-indigo-50 dark:from-blue-900/20 dark:to-indigo-900/20 rounded-lg border border-blue-200 dark:border-blue-800">
      ...
      <p className="text-sm text-blue-700 dark:text-blue-400 mb-3">
        Click to run AI validation before submission. The AI will check for completeness, appropriate N/A usage, and provide recommendations.
      </p>
    ...
    <ProtectedRoute requireAuth={true}>
      ...
      <IncidentSubmissionModal
        ...
        onViewIncident={() => {
          ...
          if (submittedIncidentData?.id) {
            router.push(`/incidents/${submittedIncidentData.id}`);
          }
        }}
        onGoToDashboard={() => {
          setSubmissionModalOpen(false);
          ...
          <div className="bg-white dark:bg-slate-800 rounded-xl shadow-lg p-4 sm:p-6 lg:p-8 border border-gray-200 dark:border-slate-700">
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 mb-2">
              <h1 className="text-xl sm:text-2xl lg:text-3xl font-bold text-gray-900 dark:text-white">
                {isEditMode ? `Edit Draft: ${editingIncidentNumber}` : 'Create New Incident'}
              </h1>
              {isEditMode && (
                <span className="px-2 sm:px-3 py-0.5 sm:py-1 rounded-full text-xs sm:text-sm font-medium bg-gray-100 text-gray-800 dark:bg-gray-700 dark:text-gray-200 self-start sm:self-center">
                  DRAFT
                </span>
              )}
            </div>
            ...
            <div className="mb-6 sm:mb-8">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 sm:gap-4">
                ...
                >
                  <div className="text-2xl sm:text-4xl mb-1.5 sm:mb-2">🍽️</div>
                  <h3 className="font-semibold text-gray-900 dark:text-white mb-0.5 sm:mb-1 text-sm sm:text-base">Food Safety</h3>
                  <p className="text-xs sm:text-sm text-gray-600 dark:text-gray-400">
                    Contamination, allergens, packaging issues
                  </p>
                </button>
                
                <button
                  type="button"
                  onClick={() => setFormData({ ...formData, type: 'MACHINE_EQUIPMENT', categoryId: '', mainCategoryId: '' })}
                  className={`p-4 sm:p-6 rounded-lg border-2 transition-all touch-manipulation ${
                ...
                >
                  <div className="text-2xl sm:text-4xl mb-1.5 sm:mb-2">⚙️</div>
                  <h3 className="font-semibold text-gray-900 dark:text-white mb-0.5 sm:mb-1 text-sm sm:text-base">Machine & Equipment</h3>
                  <p className="text-xs sm:text-sm text-gray-600 dark:text-gray-400">
                    Mechanical, electrical, control failures
                  </p>
                </button>

                <button
                  type="button"
                  onClick={() => setFormData({ ...formData, type: 'WORKPLACE_SAFETY', categoryId: '', mainCategoryId: '' })}
                  className={`p-4 sm:p-6 rounded-lg border-2 transition-all touch-manipulation ${
            ...
            {formData.type && (
              <>
                            ...
                            <div>
                              ...
                              {!incidentReportSubmitted && (
                                <div className="p-2 sm:p-3 bg-amber-50 dark:bg-amber-900/20 border border-amber-200 dark:border-amber-700 rounded-lg">
                                  <p className="text-xs sm:text-sm text-amber-700 dark:text-amber-300 flex items-center gap-1.5 sm:gap-2">
                                    <span>💡</span>
                ...
                {/* Step 5: AI Summary - At the end of the form */}
                <div className="mb-8">
                  ...
                  <div className="p-4 bg-gradient-to-r from-purple-50 to-blue-50 dark:from-purple-900/20 dark:to-blue-900/20 border border-purple-200 dark:border-purple-800 rounded-lg">
                      ...
                      >
                        {generatingAI ? (
                          <>
                            ...
                            {analyzingAttachments ? 'Analyzing Attachments...' : 'Generating...'}
                          </>
                        ) : (
                          <>
                    ...
                    <p className="text-sm text-gray-600 dark:text-gray-400 mb-3">
                      {stagedFiles.length > 0 
                        ? `The AI will analyze your ${stagedFiles.length} uploaded attachment(s) along with incident data to generate a comprehensive, evidence-based summary.`
                        : 'The AI will synthesize all form data including incident type, category, location, dates, and your description into a professional, objective summary suitable for official records.'
                      }
                    </p>
                    ...
                    <textarea
                      ...
                      rows={4}
                      placeholder="Click 'Generate Summary' to create an AI-powered professional summary of this incident based on all the information you've provided. You can edit it after generation."
                      className="w-full px-4 py-2 rounded-lg border border-purple-200 dark:border-purple-700 bg-white dark:bg-slate-800 text-gray-900 dark:text-white focus:ring-2 focus:ring-purple-500 placeholder-gray-400 dark:placeholder-gray-500"
                    />
                    
                    {/* AI Suggested Severity */}
                    {aiSuggestedSeverity && (
                      <div className="mt-3 flex items-center gap-2 p-2 bg-white dark:bg-slate-800 rounded-lg border border-purple-200 dark:border-purple-700">
                        <span className="text-sm text-gray-600 dark:text-gray-400">AI Suggested Severity:</span>
                    ...
                    {/* Enhanced AI Analysis Results (when attachments were analyzed) */}
                    {aiAnalysisResults && (
                      <div className="mt-4 space-y-4">
                            ...
                            <div className="flex items-center justify-between mb-2">
                              <h4 className="text-sm font-semibold text-blue-800 dark:text-blue-200 flex items-center gap-2">
                                ...
                                <span className="px-2 py-0.5 bg-blue-100 dark:bg-blue-800 text-xs rounded-full">
                                  {aiAnalysisResults.attachmentAnalysis.totalAttachments} file(s) analyzed
                                </span>
                              </h4>
                              <span className={`px-2 py-1 text-xs font-semibold rounded-full ${
                                ...
                                {aiAnalysisResults.attachmentAnalysis.individualAnalyses.map((analysis, idx) => (
                                  <div key={idx} className="flex items-start gap-2 text-xs p-2 bg-white dark:bg-slate-800 rounded">
                                    <span className="flex-shrink-0">
                                      {analysis.type === 'PHOTO' ? '🖼️' : analysis.type === 'DOCUMENT' ? '📄' : analysis.type === 'VIDEO' ? '🎥' : '🎤'}
                                    </span>
                        ...
                        {aiAnalysisResults.recommendedRCAMethodology && (
                          <div className="p-4 bg-gradient-to-r from-purple-50 to-blue-50 dark:from-purple-900/30 dark:to-blue-900/30 border border-purple-200 dark:border-purple-700 rounded-lg">
                            ...
                            <div className="mt-3 p-2 bg-white/50 dark:bg-black/20 rounded text-xs text-gray-600 dark:text-gray-400">
                              💡 <strong>Tip:</strong> This recommendation is based on the incident type, evidence analysis, and complexity of the situation. 
                    ...
                    <p className="mt-2 text-xs text-gray-500 dark:text-gray-400">
                      💡 The AI summary is based on: form fields, description, selected categories, dates, locations, {stagedFiles.length > 0 ? 'and analyzed attachment evidence.' : 'and uploaded evidence context.'}
                    </p>
                  </div>
                </div>

                {/* Final Action Buttons */}
                <div className="flex flex-wrap items-center gap-4 pt-6 border-t border-gray-200 dark:border-slate-700">
                  <button
                    type="button"
                    onClick={handleSubmitIncidentReport}
                    disabled={incidentReportSubmitting || !formData.type || !formData.categoryId || !formData.description || !formData.facilityId || (showCustomTitle && !formData.customTitle)}
                  ...
                  >
                    {incidentReportSubmitting ? 'Saving...' : '💾 Save as Draft'}
                  </button>
                  <Link
                    href="/dashboard"
