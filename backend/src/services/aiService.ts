...
/**
 * Validate one manual 5 Whys answer against incident context, evidence, and prior answers.
 */
export async function validateFiveWhysStepAnswer(params: {
  causeText: string;
  categoryName: string;
  problem: string;
  stepNumber: number;
  question: string;
  answer: string;
  previousSteps: Array<{ stepNumber: number; question: string; answer: string }>;
  incidentDescription: string;
  incidentType: string;
  evidence?: Array<{ fileName: string; type: string; transcription?: string | null }>;
}): Promise<{
  ...
  const defaultResult = {
    rating: wordCount >= 8 ? 'ACCEPTED' as const : 'SHALLOW' as const,
    score: wordCount >= 8 ? 72 : 45,
    feedback: wordCount >= 8
      ? 'This answer has enough detail for a first review. Confirm it is supported by the incident record before keeping it.'
      : 'This answer may be too short or vague. Add a specific, evidence-based reason that directly answers the question.',
    suggestedAnswer: null,
    reasoning: 'Generated by local validation because the validation service is unavailable.',
  };
  ...
  const evidenceText = params.evidence && params.evidence.length > 0
    ? params.evidence
      .slice(0, 8)
      .map((item, index) => {
        const transcription = item.transcription
          ? ` Notes: ${sanitizeForPrompt(item.transcription, { maxLength: 400, context: 'five-whys-step-evidence-transcription' })}`
          : '';
        return `${index + 1}. ${sanitizeForPrompt(item.fileName, { maxLength: 160, context: 'five-whys-step-evidence-file' })} (${sanitizeForPrompt(item.type, { maxLength: 60, context: 'five-whys-step-evidence-type' })}).${transcription}`;
      })
      .join('\n')
    : 'No saved evidence is attached.';

  const priorAnswerText = params.previousSteps.length > 0
    ? params.previousSteps
      .map(step => `Why ${step.stepNumber}: ${sanitizeForPrompt(step.question, { maxLength: 300, context: 'five-whys-step-prior-question' })}\nAnswer: ${sanitizeForPrompt(step.answer, { maxLength: 500, context: 'five-whys-step-prior-answer' })}`)
      .join('\n\n')
    : 'This is the first 5 Whys answer. There are no previous answers yet.';
  ...
  try {
    const completion = await openai.chat.completions.create({
      ...
      messages: [
        {
          role: 'system',
          content: `You are a senior root cause analysis reviewer. Validate ONE answer in a 5 Whys chain.

Rules:
- Use only the incident details, evidence summary, current cause, current question, and previous answers provided.
- Judge whether the answer directly answers this specific Why question.
- Consider the step number. Why 1 should explain the selected cause. Why 2-5 should logically follow from the previous answer.
- Look for shallow answers: vague wording, blame without a process reason, guesses, circular logic, or answers that move away from the incident facts.
- Do not invent unrelated scenarios. Suggestions must be realistic for this exact incident and cause.
- If evidence is limited, say what can be inferred and keep the suggested answer grounded in the record.
...
Return JSON only:
{
  "rating": "ACCEPTED" | "SHALLOW",
  "score": 0-100,
  "feedback": "1-2 short sentences explaining the rating",
  "suggestedAnswer": "A better answer grounded in the provided incident data, or null if the user's answer is strong",
  "reasoning": "Short internal reason for the rating"
}`,
        },
        {
          role: 'user',
          content: `=== INCIDENT CONTEXT ===
Incident Type: ${sanitizeForPrompt(params.incidentType, { maxLength: 120, context: 'five-whys-step-incident-type' })}
Incident Description: ${sanitizeForPrompt(params.incidentDescription, { maxLength: 1200, context: 'five-whys-step-incident-description' })}
Problem Statement: ${sanitizeForPrompt(params.problem, { maxLength: 1000, context: 'five-whys-step-problem' })}
Selected Cause Category: ${sanitizeForPrompt(params.categoryName, { maxLength: 100, context: 'five-whys-step-category' })}
Selected Cause: ${sanitizeForPrompt(params.causeText, { maxLength: 600, context: 'five-whys-step-cause' })}

=== SAVED EVIDENCE ===
${evidenceText}

=== PREVIOUS 5 WHYS ANSWERS ===
${priorAnswerText}

=== ANSWER TO VALIDATE ===
Step Number: ${params.stepNumber}
Question: ${sanitizeForPrompt(params.question, { maxLength: 500, context: 'five-whys-step-question' })}
User Answer: ${sanitizeForPrompt(params.answer, { maxLength: 800, context: 'five-whys-step-answer' })}

Validate this single answer. Decide if it is Accepted or Shallow and provide a better answer only when useful.`,
        },
      ],
      temperature: 0.2,
    ...
    const response = completion.choices[0]?.message?.content || '';
    const parsed = JSON.parse(response);
    const rating = parsed.rating === 'ACCEPTED' ? 'ACCEPTED' : 'SHALLOW';
    const score = Math.max(0, Math.min(100, Number(parsed.score ?? (rating === 'ACCEPTED' ? 75 : 45))));

    return {
      rating,
      score,
      feedback: parsed.feedback || defaultResult.feedback,
      suggestedAnswer: parsed.suggestedAnswer || null,
      reasoning: parsed.reasoning,
    };
  } catch (error: any) {
    console.error('Validate 5 Whys step answer failed:', error.message);
    return defaultResult;
  }
}
...
interface FishboneRootCauseCandidate {
  causeId: string;
  categoryName: string;
  causeText: string;
  rootCause: string;
...
interface FishboneRootCauseValidationSelection {
  causeId: string;
  categoryName: string;
  causeText: string;
  rootCause: string;
...
interface FishboneRootCauseValidationResult {
  overallDecision: string;
  problemResolutionSummary: string;
  recommendedCombination: string;
  selections: FishboneRootCauseValidationSelection[];
...
/**
 * Validate analyzed Fishbone causes and identify the root causes most likely to resolve the problem.
 */
export async function validateFishboneRootCauseSelections(params: {
  problem: string;
  analyzedCauses: FishboneRootCauseCandidate[];
  incidentContext: {
    description: string;
    type: string;
    severity?: string | null;
...
}): Promise<FishboneRootCauseValidationResult> {
  const openai = getOpenAIClient();

  if (!openai) {
    throw new Error('Cause validation requires the analysis service, but it is not configured.');
  }

  const evidenceText = (params.incidentContext.evidence || [])
    .slice(0, 12)
    .map((item, index) => {
      const notes = item.transcription
        ? ` Notes: ${sanitizeForPrompt(item.transcription, { maxLength: 500, context: 'fishbone-root-cause-evidence-notes' })}`
        : '';
      return `${index + 1}. ${sanitizeForPrompt(item.fileName, { maxLength: 160, context: 'fishbone-root-cause-evidence-file' })} (${sanitizeForPrompt(item.type, { maxLength: 80, context: 'fishbone-root-cause-evidence-type' })}).${notes}`;
    })
    .join('\n') || 'No evidence files or notes were provided.';

  const causesText = params.analyzedCauses
    .map((cause, index) => {
      const steps = (cause.fiveWhysSteps || [])
        .filter(step => step.answer?.trim())
        .map(step => `      Why ${step.stepNumber}: ${sanitizeForPrompt(step.question || 'Why?', { maxLength: 240, context: 'fishbone-root-cause-step-question' })}\n      Answer: ${sanitizeForPrompt(step.answer, { maxLength: 500, context: 'fishbone-root-cause-step-answer' })}`)
        .join('\n');

      return `${index + 1}. Cause ID: ${cause.causeId}
   Category: ${sanitizeForPrompt(cause.categoryName, { maxLength: 120, context: 'fishbone-root-cause-category' })}
   Cause: ${sanitizeForPrompt(cause.causeText, { maxLength: 500, context: 'fishbone-root-cause-cause' })}
   Root Cause Final Answer: ${sanitizeForPrompt(cause.rootCause, { maxLength: 700, context: 'fishbone-root-cause-root' })}
${steps ? `   5 Whys Chain:\n${steps}` : '   5 Whys Chain: Not provided beyond the final answer.'}`;
    })
    .join('\n\n');
  ...
  const completion = await openai.chat.completions.create({
    ...
    messages: [
      {
        role: 'system',
        content: `You are a senior root cause analysis facilitator for food safety and manufacturing incidents. Review only the data provided. Do not invent facts, hidden evidence, departments, equipment failures, or actions that are not present in the incident data.

Your job is to decide which analyzed causes, if corrected, would most likely prevent the stated problem from recurring. Some causes may be true observations but not controlling root causes. Mark those as unlikely and explain why in practical, simple language.

Use real RCA judgment:
- A likely cause should directly control or strongly prevent the problem.
- Causes can work together; identify useful combinations.
- If the data is weak, say what is missing instead of guessing.
- Explain decisions like a human QA/RCA leader, not like a generic checklist.`,
      },
      {
        role: 'user',
        content: `Problem Statement:
${sanitizeForPrompt(params.problem, { maxLength: 1200, context: 'fishbone-root-cause-problem' })}

Incident Context:
- Type: ${sanitizeForPrompt(params.incidentContext.type || 'Not specified', { maxLength: 120, context: 'fishbone-root-cause-type' })}
- Severity: ${sanitizeForPrompt(params.incidentContext.severity || 'Not specified', { maxLength: 80, context: 'fishbone-root-cause-severity' })}
- Category: ${sanitizeForPrompt(params.incidentContext.categoryName || 'Not specified', { maxLength: 120, context: 'fishbone-root-cause-incident-category' })}
- Facility: ${sanitizeForPrompt(params.incidentContext.facilityName || 'Not specified', { maxLength: 120, context: 'fishbone-root-cause-facility' })}
- Area: ${sanitizeForPrompt(params.incidentContext.areaName || 'Not specified', { maxLength: 120, context: 'fishbone-root-cause-area' })}
- Line: ${sanitizeForPrompt(params.incidentContext.lineName || 'Not specified', { maxLength: 120, context: 'fishbone-root-cause-line' })}
- Description: ${sanitizeForPrompt(params.incidentContext.description || 'Not provided', { maxLength: 1400, context: 'fishbone-root-cause-description' })}
- Existing summary: ${sanitizeForPrompt(params.incidentContext.aiSummary || 'Not provided', { maxLength: 1000, context: 'fishbone-root-cause-summary' })}
- Immediate actions: ${sanitizeForPrompt(params.incidentContext.immediateActionsTaken || 'Not provided', { maxLength: 800, context: 'fishbone-root-cause-actions' })}
...
{
  "overallDecision": "Short plain-English decision",
  "problemResolutionSummary": "Explain whether selected causes would resolve the problem alone or together.",
  "recommendedCombination": "Describe the causes that should be addressed together and why.",
  "selections": [
    {
      "causeId": "exact id from input",
      "categoryName": "category",
      "causeText": "cause text",
      "rootCause": "root cause final answer",
      "classification": "likely or unlikely",
      "confidence": 0-100,
      "reason": "specific reason using only provided data",
      "implementationImpact": "what would change if this root cause is addressed",
      "linkedCauseIds": ["cause ids that work with this cause, if any"]
    }
  ],
  "likelyCauseIds": ["ids classified likely"],
  "unlikelyCauseIds": ["ids classified unlikely"],
  "limitations": ["missing data or uncertainty, if any"]
}`,
      },
    ],
    temperature: 0.2,
  ...
  if (!content) {
    throw new Error('The analysis service returned no content.');
  }

  const parsed = JSON.parse(content);
  const inputById = new Map(params.analyzedCauses.map(cause => [cause.causeId, cause]));
  const selections: FishboneRootCauseValidationSelection[] = Array.isArray(parsed.selections)
    ? parsed.selections
      .filter((selection: any) => selection?.causeId && inputById.has(selection.causeId))
      .map((selection: any) => {
        const original = inputById.get(selection.causeId)!;
        const classification = selection.classification === 'unlikely' ? 'unlikely' : 'likely';
        const confidence = Number.isFinite(Number(selection.confidence))
          ? Math.max(0, Math.min(100, Number(selection.confidence)))
          : 70;

        return {
          causeId: original.causeId,
          categoryName: original.categoryName,
          causeText: original.causeText,
          rootCause: original.rootCause,
          classification,
          confidence,
          reason: typeof selection.reason === 'string' ? selection.reason : 'No specific reason was returned.',
          implementationImpact: typeof selection.implementationImpact === 'string' ? selection.implementationImpact : '',
          linkedCauseIds: Array.isArray(selection.linkedCauseIds)
            ? selection.linkedCauseIds.filter((id: string) => inputById.has(id))
            : [],
        };
      })
    : [];

  const coveredIds = new Set(selections.map(selection => selection.causeId));
  params.analyzedCauses.forEach(cause => {
    if (!coveredIds.has(cause.causeId)) {
      selections.push({
        ...
        confidence: 50,
        reason: 'The analysis did not return enough support to classify this as a controlling root cause.',
        implementationImpact: 'Manual review is needed before relying on this cause.',
        linkedCauseIds: [],
      });
    }
  });

  const likelyCauseIds = selections.filter(selection => selection.classification === 'likely').map(selection => selection.causeId);
  const unlikelyCauseIds = selections.filter(selection => selection.classification === 'unlikely').map(selection => selection.causeId);

  return {
    overallDecision: typeof parsed.overallDecision === 'string' ? parsed.overallDecision : 'Cause validation complete.',
    problemResolutionSummary: typeof parsed.problemResolutionSummary === 'string' ? parsed.problemResolutionSummary : '',
    recommendedCombination: typeof parsed.recommendedCombination === 'string' ? parsed.recommendedCombination : '',
    selections,
    likelyCauseIds,
    unlikelyCauseIds,
    limitations: Array.isArray(parsed.limitations) ? parsed.limitations.filter((item: any) => typeof item === 'string') : [],
  };
}
...
export async function generateCorrectiveActions(
  ...
  existingPreventiveControls?: PreventiveControlItem[]
  ...
  try {
    const prompt = buildCorrectiveActionsPrompt(
      problem,
      analyzedCauses,
      incidentContext,
      existingActions,
      existingPreventiveControls
    );
...
function buildCorrectiveActionsPrompt(
  ...
  if (incidentContext.incidentNumber) parts.push(`Incident Number: ${incidentContext.incidentNumber}`);
  if (incidentContext.incidentDate) parts.push(`Incident Date: ${incidentContext.incidentDate}`);
  if (incidentContext.shiftTime) parts.push(`Shift: ${incidentContext.shiftTime}`);
  ...
  if (incidentContext.areaName) parts.push(`Area: ${incidentContext.areaName}`);
  if (incidentContext.lineName) parts.push(`Line: ${incidentContext.lineName}`);
  if (incidentContext.immediateActionsTaken) parts.push(`Immediate Actions Already Taken: ${incidentContext.immediateActionsTaken}`);
  if (incidentContext.aiSummary) parts.push(`Incident Summary: ${incidentContext.aiSummary}`);
  if (incidentContext.evidence && incidentContext.evidence.length > 0) {
    parts.push('Evidence:');
    incidentContext.evidence.forEach((evidence, idx) => {
      parts.push(`  ${idx + 1}. ${evidence.fileName} (${evidence.type})${evidence.transcription ? ` - ${evidence.transcription}` : ''}`);
    });
  }
  ...
  analyzedCauses.forEach((cause, idx) => {
    ...
    if (cause.resolutionClassification) {
      parts.push(`   Root Cause Review: ${cause.resolutionClassification}`);
    }
    if (cause.resolutionReason) {
      parts.push(`   Review Reason: ${cause.resolutionReason}`);
    }
    if (cause.implementationImpact) {
      parts.push(`   Expected Impact: ${cause.implementationImpact}`);
    }
  ...
  if (existingPreventiveControls && existingPreventiveControls.length > 0) {
    parts.push('\n=== EXISTING PREVENTIVE CONTROLS (for context) ===');
    existingPreventiveControls.forEach((control, idx) => {
      parts.push(`${idx + 1}. ${control.control} [${control.type}] - ${control.description}`);
    });
  }
  ...
  parts.push('7. Use only the incident details, evidence, root cause review, and 5 Whys data provided above; do not invent unsupported facts.');
  parts.push('8. Fill the corresponding fields with practical, audit-ready text. Include owners and target dates only when the data supports them; otherwise leave those fields blank.');
